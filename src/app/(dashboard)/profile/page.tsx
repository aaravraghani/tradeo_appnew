...
export default function OnboardingPage() {
  ...
  const [checking, setChecking] = useState(true) // checking if already onboarded
  ...
  // On mount: call GET /api/onboarding — this sets the cookie if already done
  // and redirects to /home so existing users skip onboarding
  useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.onboardingCompleted) {
            // Cookie is now set by the API — go straight to dashboard
            router.replace('/home')
            return
          }
        }
      } catch {
        // If check fails, just show onboarding — better than a blank screen
      } finally {
        setChecking(false)
      }
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Cookie is set by the API response — middleware will now let us through
      router.push('/home')
    } catch (err: unknown) {
  ...
  // Show a blank screen while checking — avoids flash of onboarding UI
  if (checking) {
    return (
